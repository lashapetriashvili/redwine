...
(function ($) {
    ...
    var _methods = {
        init: function (options) {
            ...
            if (target.length > 0) {
                ...
                $(tbWhole).empty().addClass(settings.useSmallTable ? 'table table-sm' : 'table').appendTo(tbWrap);
                ...
                */
                ...
                if (settings.hideButtons.append && settings.hideButtons.removeLast
                    ...
                    if (!settings.hideButtons.append) {
                        var button = createGridButton(settings.customGridButtons.append, 'octicon-plus')
                    ...
                    if (!settings.hideButtons.removeLast) {
                        var button = createGridButton(settings.customGridButtons.removeLast, 'octicon-dash')
                    ...
                    if (settings.customFooterButtons && settings.customFooterButtons.length) {
                        ...
                        for (var y = settings.customFooterButtons.length - 1; y >= 0; y--) {
                            ...
                            if (buttonCfg && buttonCfg.uiButton && buttonCfg.click && buttonCfg.atTheFront) {
                                $(btnGroup).prepend(makeCustomBottomButton(tbWhole, buttonCfg));
                            }
                        }
                        // Add end buttons
                        for (var y = 0; y < settings.customFooterButtons.length; y++) {
                            var buttonCfg = settings.customFooterButtons[y];
                            if (buttonCfg && buttonCfg.uiButton && buttonCfg.click && !buttonCfg.atTheFront) {
                                $(btnGroup).append(makeCustomBottomButton(tbWhole, buttonCfg));
                            }
                        }
                    }
                }
        ...
        for (var z = 0; z < numOfRow; z++) {
            ...
            if (!settings._hideLastColumn || settings.columns.length > settings._visibleCount) {
                ...
                if (!settings.hideButtons.insert) {
                    var button = createGridButton(settings.customGridButtons.insert, 'octicon-reply')
                ...
                if (!settings.hideButtons.remove) {
                    var button = createGridButton(settings.customGridButtons.remove, 'octicon-trashcan')
                ...
                if (!settings.hideButtons.moveUp) {
                    var button = createGridButton(settings.customGridButtons.moveUp, 'octicon-arrow-up')
                ...
                if (!settings.hideButtons.moveDown) {
                    var button = createGridButton(settings.customGridButtons.moveDown, 'octicon-arrow-down')
    ...
    function createGridButton(param, octicon) {
        ...
        if (param) {
            if ($.isFunction(param)) {
                ...
                if (0 == param.indexOf('octicon-')) {
                    // Custom octicon
                    genButton = $('<button/>').attr({ type: 'button' }).addClass('btn btn-primary btn-simple btn-xs').append($('<span></span>').addClass('octicon ' + param));
                } else {
                    // HTML code
                    genButton = $(param);
                }
        ...
        if (!genButton) {
            // Use default setting (Bootstrap Button + Octicon) if button is not created.
            genButton = $('<button/>').attr({ type: 'button' }).addClass('btn btn-primary btn-simple btn-xs').append($('<span></span>').addClass('octicon ' + octicon));
        }
        return genButton;
    }
