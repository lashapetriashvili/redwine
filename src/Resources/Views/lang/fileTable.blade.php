...
@section('navbar-title', strtoupper($directorie . '/' . $language))
        ...
        <div class="card-content table-responsive">
            <table class="table table-hover">
                <thead class="text-pink text-uppercase">
                    ...
                    <th>{{ Redwine::userLang('redwine.language.language') }}</th>
                    <th>{{ Redwine::userLang('redwine.language.file') }}</th>
                ...
                <tbody>
                    @if(Redwine::bladePermissionFail('read_language'))
                        @foreach($files as $key => $file)
                            <tr>
                                <td>{{ ++$key }}</td>
                                <td class="text-uppercase">{{ $directorie }}</td>
                                <td class="text-uppercase">{{ $language }}</td>
                                <td class="text-uppercase">{{ substr(basename($file), 0, -5) }}</td>
                                <td>
                                    @if(Redwine::bladePermissionFail('read_language'))
                                        <a href="/redwine/lang/word/read/{{ $directorie }}/{{ $language }}/{{ substr(basename($file), 0, -5) }}" rel="tooltip" class="btn btn-warning btn-simple btn-xs">
                                            <i class="material-icons">visibility</i>
                                        </a>
                                    @endif
                                    @if(Redwine::bladePermissionFail('edit_language'))
                                        <button rel="tooltip" class="btn btn-primary btn-simple btn-xs" @click="editTable('{{ basename($file) }}')">
                                            <i class="material-icons">edit</i>
                                        </button>
                                    @endif
                                    @if(Redwine::bladePermissionFail('delete_language'))
                                        <button type="button" rel="tooltip" class="btn btn-danger btn-simple btn-xs" @click="deleteTable('{{ basename($file) }}')">
        ...
        vue = new Vue({
            ...
            methods: {
                @if(Redwine::bladePermissionFail('add_language'))
                    addTable: function () {
                        swal({
                            title: '{{ Redwine::userLang('redwine.language.add file') }}',
                            input: 'text',
                            ...
                            if (result.value) {
                                ...
                                this.$http.post('/redwine/lang/file/add/{{ $directorie }}/{{ $language }}/' + result.value, data).then(function (data) {
                                    if (data.body == 0) {
                                        swal({
                                            type: 'success',
                ...
                @if(Redwine::bladePermissionFail('edit_language'))
                    editTable: function (name) {
                        ...
                        swal({
                            title: '{{ Redwine::userLang('redwine.language.edit file') }}',
                            input: 'text',
                            ...
                            if (result.value) {
                                ...
                                this.$http.post('/redwine/lang/file/edit/{{ $directorie }}/{{ $language }}/' + name + '/' + result.value, data).then(function (data) {
                                    if (data.body) {
                                        swal({
                                            type: 'success',
                                            showConfirmButton: false,
                    ...
                    deleteTable: function (name) {
                        ...
                        swal({
                            ...
                            if (result.value) {
                                ...
                                this.$http.post('/redwine/lang/file/delete/{{ $directorie }}/{{ $language }}/' + name, data).then(function (data) {
                                    if (data.body) {
                                        swal({
                                            type: 'success',
                                            showConfirmButton: false,
