...
@section('title', $customPage->display_name)
                            ...
                            @foreach($customPageRow as $row)
                                @if($row->column_browse)
                                    ...
                                    @elseif($row->type == 'image')
                                        @if( $post[$row->field] )
                                            @if( File::exists( 'uploads/' . $post[$row->field] ) )
                                                <td><img src="{{ asset('uploads/' . $post[$row->field]) }}" style="width:200px"></td>
                                            @else
                                                <td style="color:#999">სურათი ვერ მოიძებნა</td>
                                            @endif
                                        @else
                                            <td style="color:#999">სურათი ვერ მოიძებნა</td>
                                        @endif
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{{ $customPage->display_name }}</h4>
                    </div>
                    <div class="modal-body" v-for="row in rows" v-if="row.column_read">
                        <div class="read-typo">
                            <div v-if="row.type == 'image'">
                                <span class="read-note text-uppercase" v-if="row.display_name">@{{ row.display_name }}</span>
                                <span class="read-note text-uppercase" v-else style="color:#999">უსათაურო</span>
                                <img :src="'/uploads/' + posts[row.field]" @error="imageLoadError" style="width: 100%" v-if="imageErorr">
                                <p class="text-muted" v-else style="color:#999">სურათი ვერ მოიძებნა</p>
                            </div>
