...
@section('title', Redwine::userLang('redwine.pages.edit'))
                ...
                @foreach($customPageRows as $customPageRow)
                    @if($customPageRow->type == 'text')
                        ...
                        @if(isset(json_decode($customPageRow->details, true)['slug']))
                                <div class="form-group">
                                    @if(isset(json_decode($customPageRow->details, true)['slug']['number']))
                                        <input type="text" class="form-control" name="{{ $customPageRow->field }}" form="add-form" placeholder="{{ $controller->lang($customPageRow->display_name, $slug, true) }}" value="{{ $controller->get($customPageRow->details) }}" @change="slug('{{ json_decode($customPageRow->details, true)['slug']['number'] }}', '{{ $customPageRow->field }}')">
                                    @else
                                        <input type="text" class="form-control" name="{{ $customPageRow->field }}" form="add-form" placeholder="{{ $controller->lang($customPageRow->display_name, $slug, true) }}" value="{{ $controller->get($customPageRow->details) }}" @change="slug(35, '{{ $customPageRow->field }}')">
                                    @endif
                        ...
                        @else
                                <div class="form-group label-floating">
                                    <label class="control-label">{{ $controller->lang($customPageRow->display_name, $slug, true) }}</label>
                    ...
                    @if($customPageRow->type == 'password')
                        ...
                        @endif
                            <div class="form-group label-floating">
                                <label class="control-label">{{ $controller->lang($customPageRow->display_name, $slug, true) }}</label>
                                <input type="password" class="form-control" name="{{ $customPageRow->field }}" form="add-form">
                            </div>
                        </div>
                        ...
                        @endif
                            <div class="form-group label-floating">
                                <label class="control-label">{{ Redwine::userLang('redwine.pages.repeat', ['input' => $controller->lang($customPageRow->display_name, $slug, true)]) }}</label>
                                <input type="password" class="form-control" id="password-{{ $customPageRow->field }}" form="add-form">
                            </div>
                        </div>
                    @endif
            ...
            @if($customPageRow->type == 'textarea (editor)')
                <div class="card" data-border="purple">
                    ...
                    <div class="card-content table-responsive table-responsive-overflow" style="padding: 10px 0 0">
                        <h6 class="card-title text-gray text-center">{{ $controller->lang($customPageRow->display_name, $slug, true) }}</h6>
                        <input type="hidden" name="{{ $customPageRow->field }}" class="editor-{{ $key }}" form="add-form" value="{{ $news[0][$customPageRow->field] }}">
                        <div id="editor-{{ $key }}" style="height: 300px;">
                            {!! $news[0][$customPageRow->field] !!}
                        </div>
                    </div> <!-- .card-content -->
                </div> <!-- .card -->
            @endif
        ...
        @foreach($customPageRows as $key => $customPageRow)
            @if($customPageRow->type == 'tinytextarea (editor)')
                <div class="card" data-border="purple">
                    ...
                    <div class="card-content table-responsive table-responsive-overflow" style="padding: 10px 0 0">
                        <h6 class="card-title text-gray text-center">{{ $controller->lang($customPageRow->display_name, $slug, true) }}</h6>
                        <input type="hidden" name="{{ $customPageRow->field }}" class="mini-editor-{{ $key }}" form="add-form" value="{{ $news[0][$customPageRow->field] }}">
                    ...
                    @if($customPageRow->type == 'select')
                        <div class="form-group" >
                            <select class="form-control" name="{{ $customPageRow->field }}" form="add-form">
                                <option value="">{{ $controller->lang($customPageRow->display_name, $slug, true) }}</option>
                                @if(isset(json_decode($customPageRow->details, true)['language']))
                                    @foreach($controller->language(json_decode($customPageRow->details, true)['language']) as $key => $select)
                                        <option value="{{ basename($select) }}" class="text-uppercase">{{ basename($select) }}</option>
                                    @endforeach
                                @endif
                    ...
                    @if($customPageRow->type == 'checkbox')
                        <div class="form-group togglebutton">
                            <label>
                                ...
                                {{ $controller->lang($customPageRow->display_name, $slug, true) }}
                            </label> 
                        </div>
                    @endif
                @endforeach
                <form action="/redwine/page/{{ $customPage->table_name }}/{{ $id }}/edit" method="post" enctype="multipart/form-data" id="add-form">
                    @if(isset($id))
                        <input type="hidden" name="edit_id" value="{{ $id }}">
                    @endif
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button class="btn btn-pink">{{ Redwine::userLang('redwine.pages.edit') }}</button>
                </form>
            </div> <!-- .card-content -->
        </div> <!-- .card -->
                ...
                <div class="card" data-border="purple">
                    ...
                    <div class="card-content table-responsive">
                        ...
                        <div class="box text-center image_{{ $key }}">
                            ...
                            <h6 class="card-title text-gray">{{ $controller->lang($customPageRow->display_name, $slug, true) }}</h6>
                        ...
                        <div class="upload-image show_image_{{ $key }}" style="display:none" @click="removeImage({{ $key }}, '{{ $customPageRow->field }}')">
                            ...
                            <input type="hidden" name="edit_image_{{ $key }}" form="add-form">
                            <div class="close-image">{{ Redwine::userLang('redwine.pages.choose different image') }}</div>
                        </div>
            ...
            <div class="card-content table-responsive tag-card">
                ...
                @foreach($customPageRows as $key => $customPageRow)
                    ...
                    @if($customPageRow->type == 'seo textarea')
                        <div class="form-group label-floating">
                            <label class="control-label">{{ $controller->lang($customPageRow->display_name, $slug, true) }}</label>
                            <textarea class="form-control" rows="5" name="{{ $customPageRow->field }}" form="add-form">a</textarea>
                        </div>
                    @endif
...
@push('scripts')
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            methods: {
                ...
                removeImage: function (number, name) {
                    ...
                    $( "input[name='edit_image_" + number + "']" ).val('');
                },
                ...
                unique: function (field, displayName) {
                    ...
                    this.$http.post("{{ URL::to('redwine/page/'. $customPage->table_name .'/check/unique')}}", data).then(function(json){
                        if (json.body) {
                            redwine.note('danger', 'notifications', displayName + ' {{ Redwine::userLang('redwine.pages.already exists') }}');
        ...
        @foreach ($customPageRows as $key => $customPageRow)
            @if($customPageRow->type == 'text')
                $( "input[name='{{ $customPageRow->field }}']" ).val('{{ $news[0][$customPageRow->field] }}');
            @endif

            @if($customPageRow->type == 'textarea')
                $( "textarea[name='{{ $customPageRow->field }}']" ).val('{{ $news[0][$customPageRow->field] }}');
            @endif

            @if($customPageRow->type == 'select')
                $( "select[name='{{ $customPageRow->field }}']" ).val('{{ $news[0][$customPageRow->field] }}');
            @endif

            @if($customPageRow->type == 'image')
                @if($news[0][$customPageRow->field])
                    $('.{{ $customPageRow->field }}').attr('src', '{{ asset( 'uploads/images/' . $news[0][$customPageRow->field] ) }}');
                    $("input[name='edit_image_{{ $key }}']").val('{{ $news[0][$customPageRow->field] }}');
                    $('.image_{{ $key }}').hide();
                    $('.show_image_{{ $key }}').show();
                @endif
            @endif
            ...
            @if($customPageRow->type == 'checkbox')
                @if($news[0][$customPageRow->field] == 1)
                    $( "input[name='{{ $customPageRow->field }}']" ).prop('checked', true);
                @endif
            @endif
                ...
                @if(isset(json_decode($customPageRow->details, true)['required']))
                    @if($customPageRow->type == 'seo textarea' || $customPageRow->type == 'textarea')
                        if ( $( "textarea[name='{{ $customPageRow->field }}']" ).val() == '' ) {
                            redwine.note('danger', 'notifications', '{{ Redwine::userLang('redwine.pages.required', ['input' => $controller->lang($customPageRow->display_name, $slug, true)]) }}!');
                    ...
                    @elseif($customPageRow->type == 'password')
                        if ($( "input[name='{{ $customPageRow->field }}']" ).val() != $( "#password-{{ $customPageRow->field }}" ).val()) {
                            redwine.note('danger', 'notifications', '{{ Redwine::userLang('redwine.pages.password required', [
                                    'password'       => $controller->lang($customPageRow->display_name, $slug, true),
                                    'repeat password' => $controller->lang($customPageRow->display_name, $slug, true)
                                ]) }}');
                            event.preventDefault();
                        }
        ...
        @foreach($customPageRows as $key => $customPageRow)
            @if($customPageRow->type == 'seo tag')
                tag = {!! json_encode( explode(',', $news[0][$customPageRow->field]) ) !!};
                var tag_{{ $key }} = new Taggle('tag-{{ $key }}', {
                    ...
                    @if($customPageRow->display_name)
                        @if(isset($id))
                            placeholder: false,
                        @else
                            placeholder: '{{ $controller->lang($customPageRow->display_name, $slug, true) }}',
                        @endif
                    @else
                        @if(isset($id))
                            placeholder: false,
                        @else
                            placeholder: '{{ Redwine::userLang('redwine.pages.tags') }}',
                        @endif
                    @endif
                ...
                $( '.tag-{{ $key }}' ).val('{{ $news[0][$customPageRow->field] }}');
            @endif
        @endforeach
