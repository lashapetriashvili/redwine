...
class MenuController extends Controller
{
    ...
    public function menu($name, $detal = [])
    {
        ...
        foreach ($items as $item) {
            if ($item['parent'] == 0) {
                ...
                $html .= "<li". $this->menuClass($detal, $link) ."><a href='$link'>". $this->menuIcon($item->icon) ."<p>" . $this->lang($item->label, $detal) . "</p></a>";
                $id = $item->id;
    ...
    protected function lang($text, $detal)
    {
        if (substr($text, 0, 1) == '{' && substr($text, -1) == '}') {
            if (isset($detal['language'])) {
                return Redwine::lang(substr($text, 1, -1), [], $detal['language']);
            } else {
                return Redwine::lang(substr($text, 1, -1));
            }
        } else {
            return $text;
        }
